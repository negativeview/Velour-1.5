...
exports.setupRoutes = function(app) {
	app.get('/user/:userId', basicUserPage);
	app.get('/user/:userId/avatar.png', fullSizedUserIcon);
...
function isLoggedInAs(req, res, next) {
	...
	if (req.session.authenticatedAs != req.user.getId()) {
		req.error = 'Permission denied.';
		console.log(req.session.authenticatedAs + ' trying to edit ' + req.user.getId());
	}
...
function basicUserPage(req, res) {
	// No user was found? 404 them.
	if (!req.user) {
		res.send(404);
		return;
	}
...
function fullSizedUserIcon(req, res) {
	if (!req.user) {
		res.send(404);
		return;
	}
	
	req.user.getFullSizedIcon(function(error, result) {
